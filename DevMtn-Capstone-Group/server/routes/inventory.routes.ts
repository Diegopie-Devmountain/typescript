...
inventoryRouter.post('/new', async (req, res) => {
    const { itemName, itemDescription, itemPrice, imageUrl, quantity, isSpecialItem } = req.body;
    const newItem: ItemData = await Inventory.create({ itemName, itemDescription, itemPrice, imageUrl, quantity, isSpecialItem });
...
inventoryRouter.delete('/:itemId', async (req, res) => {
    const { itemId } = req.params;
    const itemRemove: ItemData = await Inventory.findByPk(itemId);
